...
public class DyOpSendMail : DyOpBase
{
    ...
    {
        if (!RightMgr.getInstance().canEdit(RightDef.SVR_SEND_MAIL, user))
...
public class DyOpFishlordParamAdjust : DyOpParamAdjust
{
    ...
    {
        Dictionary<string, object> data = DBMgr.getInstance().getTableData(m_roomTableName, "room_id", roomId,
             s_fishlordFields, user.getDbServerID(), DbName.DB_GAME);
            ...
            case (int)ReloadTable.fish: // 加载鱼表
                {
                    ...
                    if (res)
                    {
                        OpLogMgr.getInstance().addLog(LogType.LOG_TYPE_RELOAD_TABLE,
                            new LogReloadTable((int)GameId.fishlord),
                            user);
                    }
                }
                break;
...
public class DyOpAddWishCurse : DyOpBase
{
    ...
    void viewWishCurse(GMUser user)
    {
        ...
        List<Dictionary<string, object>> dataList =
            DBMgr.getInstance().executeQuery(TableName.FISHLORD_PLAYER, user.getDbServerID(), DbName.DB_GAME,
            imq, 0, 0, FIELDS);
...
public class DyOpKickPlayer : DyOpBase
{
            ...
            case "dec":
                {
                    ...
                    if (res == OpRes.opres_success)
                    {
                        ...
                        if (code)
                        {
                            OpLogMgr.getInstance().addLog(LogType.LOG_TYPE_PLAYER_OP,
                                new LogPlayerOp(playerId, p.m_op=="add"?1:0,
                                    p.m_prop, oriVal),
                                user);
                        }
                        else
    ...
    void getLogFishPlayer(GMUser user)
    {
        ...
        List<Dictionary<string, object>> data = DBMgr.getInstance().getDataListFromTable(TableName.FISHLORD_PLAYER, 
            user.getDbServerID(), DbName.DB_GAME,
                "LogFish", true, new string[] { "player_id" });
    ...
    void getLimitDbSendPlayer(GMUser user)
    {
        ...
        List<Dictionary<string, object>> data = DBMgr.getInstance().getDataListFromTable(TableName.PLAYER_INFO,
            user.getDbServerID(), DbName.DB_PLAYER,
                "canSendDragonBall", false, new string[] { "player_id" });
...
public class DyOpWeekChampionControl : DyOpBase
{
    ...
    OpRes addSafeAccount(ParamGrandPrixWeekChampion p, GMUser user)
    {
        ...
        Dictionary<string, object> qd = DBMgr.getInstance().getTableData(TableName.PLAYER_INFO, "player_id", playerId,
            new string[] { "nickname", "is_robot" },
            user.getDbServerID(), DbName.DB_PLAYER);
        if (qd == null)
            return OpRes.op_res_player_not_exist;
        ...
        bool res = DBMgr.getInstance().keyExists(TableName.MATCH_GRAND_SAFE_ACCOUNT, "playerId", playerId,
            user.getDbServerID(), DbName.DB_GAME);
        if (res)
            return OpRes.op_res_failed;
    ...
    OpRes modifyBestScore(ParamGrandPrixWeekChampion p, GMUser user)
    {
        ...
        Dictionary<string, object> data1 = DBMgr.getInstance().getTableData(TableName.MATCH_GRAND_SAFE_ACCOUNT,
        ...
        Dictionary<string, object> data2 = DBMgr.getInstance().getTableData(TableName.MATCH_GRAND_PRIX_DAY,
                           "playerId", playerId,
                           new string[] { "bestScore" },
                           user.getDbServerID(), DbName.DB_GAME);
        ...
        bool res = DBMgr.getInstance().update(TableName.MATCH_GRAND_SAFE_ACCOUNT, upData, "playerId", playerId,
                                user.getDbServerID(), DbName.DB_GAME);
        return res ? OpRes.opres_success : OpRes.op_res_failed;
    }
    ...
    {
        List<Dictionary<string, object>> dataList = DBMgr.getInstance().executeQuery(TableName.MATCH_GRAND_SAFE_ACCOUNT,
            user.getDbServerID(),
            DbName.DB_GAME);
...
public class DyOpFishBoss : DyOpBase
{
    ...
    OpRes query(GMUser user)
    {
        ...
        List<Dictionary<string, object>> dataList =
            DBMgr.getInstance().executeQuery(TableName.FISHLORD_ROOM, user.getDbServerID(), DbName.DB_GAME,
            null, 0, 0, s_fields, "room_id", true);
