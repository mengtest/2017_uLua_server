...
public class DBMgr
{
    ...
    private void open()
    {
        ...
        m_mongoServer[0] = create(WebConfigurationManager.AppSettings["account"], null, true);
        m_dbServer.Add(WebConfigurationManager.AppSettings["account"], 0);
        ...
        if (!m_dbServer.ContainsKey(WebConfigurationManager.AppSettings["payment"]))
        {
    ...
    public AccountInfo loadUser(string user)
    {
        ...
        if (data != null)
        {
            ...
            if (data.ContainsKey("viewChannel"))
            {
                info.m_viewChannel = Convert.ToString(data["viewChannel"]);
            }
        }
        return info;
    }
...
//                 tmp.CreateIndex(indexname);
    ...
    public bool ExecuteUpdate(string table, string fieldname, object val, Dictionary<string, object> data)
    {
        return ExecuteUpdateByQuery(table, Query.EQ(fieldname, BsonValue.Create(val)), data);
    }

    public bool ExecuteUpdateByQuery(string table, IMongoQuery queries, Dictionary<string, object> data)
    {
        try
        {
            var cb = check_table(table);
...
//                     ret = ret.SetSortOrder(SortBy.Descending(sort));
    ...
    public MapReduceResult executeMapReduce(string table, IMongoQuery query, string map_js, string reduce_js, string outTableName = "")
    {
        try
        {
            var cb = check_table(table);
