...
public class QueryGMAccount : QueryBase
{
    ...
    public override OpRes doQuery(object param, GMUser user)
    {
        ...
        for (i = 0; i < data.Count; i++)
        {
            ...
            if (data[i].ContainsKey("viewChannel"))
            {
...
public class QueryPlayerMoneyDetail : QueryPlayerMoney
{
    ...
    protected override OpRes query(ParamMoneyQuery param, IMongoQuery imq, GMUser user)
    {
        ...
        // 查看满足条件的记当个数
        user.totalRecord = DBMgr.getInstance().getRecordCount("logPlayerInfo", imq, user.getDbServerID(), DbName.DB_PUMP);

        List<Dictionary<string, object>> data =
             DBMgr.getInstance().executeQuery("logPlayerInfo", user.getDbServerID(), DbName.DB_PUMP, imq,
                                              (param.m_curPage - 1) * param.m_countEachPage, param.m_countEachPage,
                                              null, "genTime", false);
...
//         }
    ...
    OpRes queryByRegTime(GMUser user, ParamQuery p)
    {
        DateTime mint = DateTime.Now, maxt = DateTime.Now;
        bool res = Tool.splitTimeStr(p.m_time, ref mint, ref maxt);
        if (!res)
        ...
        IMongoQuery imq = Query.And(imq1, imq2, Query.EQ("is_robot", false));

        List<Dictionary<string, object>> dataList =
             DBMgr.getInstance().executeQuery(TableName.PLAYER_INFO, user.getDbServerID(), DbName.DB_PLAYER, imq,
...
public class ResultTotalConsume
{
    ...
    public string getFishReason(int r)
    {
        ...
        if (r >= (int)FishLordExpend.fish_unlock_level_start && r <= (int)FishLordExpend.fish_unlock_level_end) 
        {
        ...
        {
            return string.Format("导弹消耗({0})", StrName.s_roomName[r - (int)FishLordExpend.fish_missile]);
        }
        return "";
    }
...
public class QueryGameRechargeByDay : QueryBase
{
    ...
    private OpRes query(GMUser user, IMongoQuery imq)
    {
        ...
        List<Dictionary<string, object>> dataList =
             ...
             DbName.DB_PAYMENT, imq, 0, 0, null, "date", false);
        ...
        for (i = 0; i < dataList.Count; i++)
        {
            ...
            foreach (var game in data.Keys)
            {
                ...
                try
                {
                    if (game == "total_rmb")
                    {
                        item.m_totalRecharge = Convert.ToInt64(data["total_rmb"]);
                    }
                    else
                    {
                        long val = Convert.ToInt64(data[game]);
                        item.add(game, val);
                ...
                {
                    CLOG.Info("QueryGameRechargeByDay:" + ex.ToString());
                }
            }
        }
...
public class ResultActivationItem : GameStatData
{
    ...
    public string get2DayRemain(bool isRecharge = false)
    {
        if (isRecharge)
        {
            return getRemainPercent(m_2DayRemainCountRecharge, m_newAccRechargePersonNum);
        }
        return getRemainPercent(m_2DayRemainCount, m_regeditCount);
    }

    public string get3DayRemain(bool isRecharge = false)
    {
        if (isRecharge)
        {
            return getRemainPercent(m_3DayRemainCountRecharge, m_newAccRechargePersonNum);
        }
        return getRemainPercent(m_3DayRemainCount, m_regeditCount);
    }
    public string get7DayRemain(bool isRecharge = false)
    {
        if (isRecharge)
        {
            return getRemainPercent(m_7DayRemainCountRecharge, m_newAccRechargePersonNum);
        }
        return getRemainPercent(m_7DayRemainCount, m_regeditCount);
    }

    public string get30DayRemain(bool isRecharge = false)
    {
        if (isRecharge)
        {
            return getRemainPercent(m_30DayRemainCountRecharge, m_newAccRechargePersonNum);
        }
...
public class QueryTdActivation : QueryBase
{
    ...
    public override OpRes doQuery(object param, GMUser user)
    {
        ...
        List<IMongoQuery> queryList = new List<IMongoQuery>();
        int condCount = 0;
        IMongoQuery imqChannel = null;
//         if (p.m_param != "")
//         {
//             queryList.Add(Query.EQ("channel", BsonValue.Create(p.m_param)));
//         }
        genChannelCond(p.m_param, user, ref imqChannel);
        if (p.m_time != "")
        {
            DateTime mint = DateTime.Now, maxt = DateTime.Now;
            bool res = Tool.splitTimeStr(p.m_time, ref mint, ref maxt);
    ...
    private OpRes query(ParamQuery param, IMongoQuery imq, GMUser user)
    {
        ...
        for (i = 0; i < dataList.Count; i++)
        {
            ...
            if (data.ContainsKey("30DayRemainCountRecharge"))
            {
                tmp.m_30DayRemainCountRecharge = Convert.ToInt32(data["30DayRemainCountRecharge"]);
            }
    ...
    public static void genChannelCond(string channelParam,
                                      GMUser user,
                                      ref IMongoQuery imqResult)
    {
        List<IMongoQuery> queryList = new List<IMongoQuery>();
        ...
        if(string.IsNullOrEmpty(channelParam))
        {
            if (!user.isAdmin())
            {
                string[] arr = Tool.split(user.m_viewChannel, ',', StringSplitOptions.RemoveEmptyEntries);
                for (int i = 0; i < arr.Length; i++)
                {
                    queryList.Add(Query.EQ("channel", BsonValue.Create(arr[i])));
                }
            }
        }
        else
        {
            queryList.Add(Query.EQ("channel", BsonValue.Create(channelParam)));
        }

        if (queryList.Count > 0)
        {
            imqResult = Query.Or(queryList);
        }
    }
}
...
public class QueryTotalPlayerMoney : QueryBase
{
    ...
    private OpRes query(ParamQuery param, IMongoQuery imq, GMUser user)
    {
        user.totalRecord = DBMgr.getInstance().getRecordCount(TableName.PUMP_PLAYER_TOTAL_MONEY, imq, user.getDbServerID(), DbName.DB_PUMP);
...
//             queryList.Add(Query.EQ("channel", BsonValue.Create(p.m_param)));
//         }
        QueryTdActivation.genChannelCond(p.m_param, user, ref imqChannel);
        if (p.m_time != "")
        {
            DateTime mint = DateTime.Now, maxt = DateTime.Now;
            bool res = Tool.splitTimeStr(p.m_time, ref mint, ref maxt);
