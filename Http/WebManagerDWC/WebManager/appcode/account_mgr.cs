...
class AccountMgr
{
    ...
    public OpRes delAccount(string account, GMUser user = null)
    {
    ...
    // 更新gm账号可查看的渠道数据
    public bool updateViewChannel(string gmAcc, string viewChannel)
    {
        Dictionary<string, object> data = new Dictionary<string, object>();
        data.Add("viewChannel", viewChannel);
        bool res = DBMgr.getInstance().update(TableName.GM_ACCOUNT, data, "user", gmAcc, 0, DbName.DB_ACCOUNT);
        return res;
    }

    // 更新gm账号可查看的渠道数据
    public bool updateGmType(string gmAcc, string gmType)
    {
        Dictionary<string, object> data = new Dictionary<string, object>();
        data.Add("type", gmType);
        bool res = DBMgr.getInstance().update(TableName.GM_ACCOUNT, data, "user", gmAcc, 0, DbName.DB_ACCOUNT);
        return res;
    }
