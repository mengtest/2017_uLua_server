...
public class CMySqlDbServer
{
    ...
    public CMySqlDbServer(string serverIp, string connStr)
    {
        m_serverIp = serverIp;
        CONNECT_STR = connStr;
    }
    ...
    public bool keyIntExists(string tableName, string field, object val, int dbName)
    {
        ...
        string sql = string.Format(CSqlStr.SQL_COUNT_INT, tableName, field, v);
    ...
    public bool keyStrExists(string tableName, string field, object val, int dbName)
    {
        ...
        string sql = string.Format(CSqlStr.SQL_COUNT_CHAR, tableName, field, v);
        MySqlDataReader r = startQuery(sql, dbName);
        if (r != null)
        {
            if (r.Read())
    ...
    {
        string sql = string.Format(CSqlStr.SQL_COUNT_WHERE, tableName, sqlCondition);
        MySqlDataReader r = startQuery(sql, dbName);
        if (r != null)
        {
            if (r.Read())
