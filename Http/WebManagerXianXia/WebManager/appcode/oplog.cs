...
{
    // 封账号
    public const int LOG_TYPE_BLOCK_ACC = 1;

    // 重置密码
    public const int LOG_TYPE_RESET_PWD = 2;

    // 祝福与诅咒
    public const int LOG_TYPE_WISH_CURSE = 3;

    // 踢出玩家
    public const int LOG_TYPE_KICK_PLAYER = 4;

    // 创建会员
    public const int LOG_TYPE_CREATE_PLAYER = 5;
    ...
    // API审批
    public const int LOG_TYPE_API_APPROVE = 10;

    public const int LOG_TYPE_MAX = 11;
}
...
// 踢出玩家
[Serializable]
class LogKickPlayer : OpParam
{
    public string m_acc = "";

    public LogKickPlayer() { }
    public LogKickPlayer(string acc)
    {
        m_acc = acc;
    }
    ...
    public override string getDescription(OpInfo info, string str)
    {
        ...
        LogKickPlayer param = BaseJsonSerializer.deserialize<LogKickPlayer>(str);
        if (param != null)
        {
            return string.Format(info.m_fmt, param.m_acc);
        }
...
// 创建会员
[Serializable]
class LogCreatePlayer : OpParam
{
    public string m_acc = "";

    public LogCreatePlayer() { }
    public LogCreatePlayer(string acc)
    {
        m_acc = acc;
    }
...
// api审批
[Serializable]
class LogApiApprove : OpParam
{
    public string m_apiAcc = "";
    public string m_aliasName;

    public LogApiApprove() { }
    public LogApiApprove(string acc, string aliasName)
    {
        m_apiAcc = acc;
