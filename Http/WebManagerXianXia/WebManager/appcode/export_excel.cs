...
public class ExportMgr : SysBase
{
    ...
    public override void initSys()
    {
        ...
        m_items.Add(ExportType.exportTypeScoreOpLog, new ExportScoreOpLog());
...
///////////////////////////////////////////////////////////////////////////////
// 导出上分下分记录
public class ExportScoreOpLog : ExportBase
{
    public override OpRes doExport(object param, GMUser user)
    {
        m_cond.startExport();
        QueryMgr mgr = user.getSys<QueryMgr>(SysType.sysTypeQuery);
        OpRes res = mgr.makeQuery(param, QueryType.queryTypeQueryScoreOpRecord, user, m_cond);
        if (res != OpRes.opres_success)
            return res;
        ...
        ep.m_account = user.m_user;
        ep.m_dbServerIP = user.m_dbIP;
        ep.m_tableName = TableName.GM_SCORE;
        ep.m_condition = m_cond.getCond();
        return RemoteMgr.getInstance().reqExportExcel(ep);
    }
}