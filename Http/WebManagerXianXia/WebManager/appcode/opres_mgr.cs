...
class OpResMgr
{
    ...
    // 取得结果串
    public string getResultString(OpRes res, object p1)
    {
        int id = (int)res;
        if (m_ops.ContainsKey(id))
        {
            OpResInfo info = m_ops[id];
            return string.Format(info.m_fmt, p1);
        }
        return "";
    }

    // 取得结果串
    public string getResultString(OpRes res, object p1, object p2)
    {
        int id = (int)res;
        if (m_ops.ContainsKey(id))
        {
            OpResInfo info = m_ops[id];
            return string.Format(info.m_fmt, p1, p2);
        }
        return "";
    }
...
public enum OpRes
{
    ...
    op_res_account_has_exists,      // 账号已存在

    op_res_account_info,            // 创建的账号为: {0}
    op_res_db_failed,               // 数据库操作失败
    op_res_no_right,                // 没有权限操作
    op_res_error_code,              // 错误号为{0}
    op_res_money_not_enough,        // 余额不足
    op_res_player_in_game,          // 玩家处于游戏中
    op_res_acc_block,               // 账号被停封

    op_res_player_account_info,     // 创建的账号为: {0}

    op_res_prefix_has_exists,        // 前缀重复

    op_res_wait_approve,             // 已提交，请等待管理员审批
    op_res_player_locked,            // 玩家账号被锁定，需要先解锁
    op_res_time_beyond_range,        // 时间范围大
    op_res_beyond_range,             // 超出范围

    op_res_player_in_game_cannot_subscore, // 玩家在游戏内，不能下分。
    op_res_happend_exception,              // 本操作出现异常情况
}
